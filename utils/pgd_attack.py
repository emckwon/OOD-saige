"""
Code from jfc43/robust-ood-detection
""" 
...
class RotPGDAttack:
    ...
    def perturb(self, x, batch_size, y=None):
        """
        Given examples (x, y), returns their adversarial counterparts with
        an attack length of eps.
        ...
        for ii in range(self.nb_iter):
            ...
            outputs, pens = self.model(adv_x)
            rot_outputs = self.model.rot_head(pens[:4*batch_size])
            loss = self.loss_func(outputs[:batch_size], y[:batch_size])
            loss += self.loss_func(rot_outputs, y[batch_size:])
