...
outf=os.path.join(cfg['exp_root'],cfg['exp_dir'],'valid','MD_ADV_untied_cov')
...
#             count += 1
        ...
        sample_mean, precision = lib_generation.semi_tied_sample_estimator(model, cfg['model']['num_classes'], feature_list, test_loader)
...
#         m_list = [0.0, 0.01, 0.005, 0.002, 0.0014, 0.001, 0.0005]
        for magnitude in m_list:
            ...
            for i in progress_bar:
                ...
                M_in, fooled \
                = lib_generation.get_Mahalanobis_score_adv_semi_tied(model, valid_clean_data, valid_label, \
            ...
            for i in progress_bar:
                ...
                M_out, _ \
                = lib_generation.get_Mahalanobis_score_adv_semi_tied(model, valid_adv_data, valid_label, \
                                                           cfg['model']['num_classes'], outf, cfg['model']['net_type'], \
                                                           sample_mean, precision, i, magnitude)
            ...
            for i in progress_bar:
                ...
                M_noisy, fooled \
                = lib_generation.get_Mahalanobis_score_adv_semi_tied(model, valid_noisy_data, valid_label, \
                                                           cfg['model']['num_classes'], outf, cfg['model']['net_type'], \
                                                           sample_mean, precision, i, magnitude)
            ...
            print("Magnitude {} | noise {} | Adv_attack {} => clean_fooled {} | noisy_fooled {}".format(magnitude, random_noise_size, adv_noise, clean_fooled, noisy_fooled))
            fooled_log.write("\nMagnitude {} | noise {} | Adv_attack {} => clean_fooled {} | noisy_fooled {}".format(magnitude, random_noise_size, adv_noise, clean_fooled, noisy_fooled))
        ...
        for magnitude in m_list:
            ...
            for i in progress_bar:
                ...
                M_in, fooled = lib_generation.get_Mahalanobis_score_semi_tied(model, test_loader, cfg['model']['num_classes'], outf, \
                                                            True, cfg['model']['net_type'], sample_mean, precision, i, magnitude,3)
                M_in = np.asarray(M_in, dtype=np.float32)
                if i == 0:
                    Mahalanobis_in = M_in.reshape((M_in.shape[0], -1))
                else:
            ...
            for i in progress_bar:
                ...
                M_out, _ = lib_generation.get_Mahalanobis_score_semi_tied(model, out_test_loader, cfg['model']['num_classes'], outf, \
                                                                 False, cfg['model']['net_type'], sample_mean, precision, i, magnitude,3)
                M_out = np.asarray(M_out, dtype=np.float32)
                if i == 0:
                    Mahalanobis_out = M_out.reshape((M_out.shape[0], -1))
                else:
        ...
        for magnitude in m_list:
            ...
            for i in progress_bar:
                ...
                M_out, _ = lib_generation.get_Mahalanobis_score_semi_tied(model, public_out_loader, cfg['model']['num_classes'], outf, \
                                                                 False, cfg['model']['net_type'], sample_mean, precision, i, magnitude,3)
                M_out = np.asarray(M_out, dtype=np.float32)
                if i == 0:
                    Mahalanobis_out = M_out.reshape((M_out.shape[0], -1))
