...
def get_train_stats(model, in_loader, m_cfg):
    model.eval()
    num_classes = m_cfg['num_classes']
    means = torch.zeros((num_classes, model.nChannels))
    Nc = torch.zeros(num_classes)
    N = 0
    cov_mat = torch.zeros((model.nChannels, model.nChannels))
    ...
    for cur_iter, in_set in enumerate(in_loader):
        ...
        N += len(targets)
        (_, _, f_logits) = model(data)
        f_logits = f_logits.detach().cpu()
        for i in range(len(targets)):
            #print(targets[i])
            Nc[targets[i]] += 1
            means[targets[i]] += f_logits[i]
    ...
    for cur_iter, in_set in enumerate(in_loader):
        ...
        f_logits = f_logits.detach().cpu()
        for i in range(len(targets)):
            diff = (f_logits[i] - means[targets[i]]).unsqueeze(0)
            cov_mat += torch.matmul(diff.t(), diff)
    ...
    print(means)
    print(cov_mat)
    return means, torch.inverse(cov_mat)
...
def valid_epoch_wo_outlier(model, in_loader, loss_func, cur_epoch, logfile2, attack_in=None):
    ...
    for cur_iter, in_set in enumerate(in_loader):        
        ...
        metrics.show_wrong_samples_targets(g_logits[:len(targets)], targets, logfile2)
...
def valid_epoch_w_outlier(model, in_loader, out_loader, loss_func, detector_func, cur_epoch, logfile2, attack_in=None, attack_out=None):
    ...
    for cur_iter, (in_set, out_set) in enumerate(zip(in_loader, out_loader)):        
        ...
        confidences_dict = detector_func(f_logits, targets, global_cfg['detector'])
...
def main():
    ...
    # Data Loader
    cfg['in_train_dataset'] = cfg['in_dataset'].copy()
    cfg['in_train_dataset']['split'] = 'train'
    in_train_loader = getDataLoader(ds_cfg=cfg['in_train_dataset'],
                                    dl_cfg=cfg['dataloader'],
                                    split="train")
    
    means, cov_mat_inv = get_train_stats(model, in_train_loader, cfg['model'])
    ...
    global_cfg['detector']['means'] = means
    global_cfg['detector']['cov_mat_inv'] = cov_mat_inv
