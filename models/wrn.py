...
class WideResNetPen(nn.Module):
    def __init__(self, cfg):
        super(WideResNetPen, self).__init__()
        depth = cfg['depth']
        num_classes = cfg['num_classes']
        widen_factor = cfg['widen_factor']
        dropRate = cfg['drop_rate']
...
class WideResNetPen224(nn.Module):
    def __init__(self, cfg):
        super(WideResNetPen224, self).__init__()
        depth = cfg['depth']
        num_classes = cfg['num_classes']
        widen_factor = cfg['widen_factor']
        dropRate = cfg['drop_rate']
...
class WideResNetFeat224(nn.Module):
    def __init__(self, cfg):
        super(WideResNetFeat224, self).__init__()
        depth = cfg['depth']
        num_classes = cfg['num_classes']
        widen_factor = cfg['widen_factor']
        dropRate = cfg['drop_rate']
    ...
    def forward(self, x):
        ...
        out = self.block2(out)
        out = self.block3(out)
        pen = self.relu(self.bn1(out))
        out = F.avg_pool2d(pen, 14)
        out = out.view(-1, self.nChannels)
        return self.fc(out), pen
    
    
class WideResNet256(nn.Module):
    def __init__(self, cfg):
        super(WideResNet256, self).__init__()
        depth = cfg['depth']
        num_classes = cfg['num_classes']
        widen_factor = cfg['widen_factor']
        dropRate = cfg['drop_rate']
